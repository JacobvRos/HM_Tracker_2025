# -*- coding: utf-8 -*-
'''
Title: Tracker (Headless / Excel Column Lists / Auto-Video)
Description: 1. Reads metadata from *RecordingMeta.xlsx (Handles vertical lists).
             ...
             3. Optimized for Batch/Massive Analysis (No pop-ups, no prompts).
'''
...
class Tracker:
    def __init__(self, vp, nl, out, metadata, onnx_weight):
        ...
        # Clear existing handlers to prevent duplicate logging in batch runs
        ...
        try:
            # Check for specifically named timestamp file first
    ...
    def change_name_csv(self, output_path):
        """Renames the generic stitched CSV to the specific Rat/Date CSV if it exists."""
        ...
        if os.path.exists(stitched_name):
            try:
                # If target exists, remove it first to avoid error on rename
            ...
            # It's possible the file was already renamed in a previous run or doesn't exist.
    ...
    def find_start(self, center_rat):
        ...
        if dist_to_node < 60:
            self.logger.info(f'[DEBUG] Recording Trial {self.trial_num} (dist to start: {dist_to_node:.1f}px < 60px threshold)')
            ...
            if int(self.trial_type) == 4 or int(self.trial_type) == 5:
                for n in self.special_trials:
                    if int(n) == self.trial_num:
                        self.NGL = True
                        self.start_time = (self.frame_time / (1000 * 60)) % 60
                        self.logger.info(f"[DEBUG] Trial type 4/5 (FREE ROAMING) activated for trial {self.trial_num}")
    ...
    def cnn(self, frame):
            ...
            matches = layers_result[np.where(np.max(layers_result[:, 4:], axis=1) > 0.7)] 
            ...
            if len(indexes) > 0:
                for i in indexes.flatten():
                    ...
                    color = colors[i]
                    cv2.rectangle(self.disp_frame, (x, y), (x + w, y + h), color, 2)
                    cv2.putText(self.disp_frame, f"{label} {confidence:.2f}", (x, y + 20), font, 1, (255, 255, 255), 1)

                    if label == 'head':
                        rat_candidates.append((confidence, centroid, 'head'))
                        detected_head_this_frame = True
                    elif label == 'rat':
                        if not any(isinstance(c, tuple) and len(c) == 3 and c[2] == 'head' for c in rat_candidates):
            ...
            if self.Researcher and active_rat_pos and not self.record_detections:
                ...
                if (not self.start_trial and not self.end_session and 
                    ...
                    # DEBUG: Log this transition
                    self.logger.info(f"[DEBUG] Advancing trial via researcher proximity: dist={dist:.1f}px <= 800px")
                    self.start_trial = True
                    self.trial_num += 1
                    self.check = False
            ...
            if (not self.start_trial and not self.record_detections and 
                ...
                if researcher_covers_start:
                    if not self.covering_start_node:
                        ...
                        self.logger.info(f"[DEBUG] Researcher started covering start node. Need to hold for {self.cover_required_time}s")
        ...
        if self.NGL:
            ...
            if minutes >= 10:
                ...
                if self.reached:
                    ...
                    self.logger.info(f"[DEBUG NGL] 10min reached but goal NOT reached - trial continues (reached={self.reached})")
    ...
    def annotate_debug_state(self, frame, x_start=10, y_start=200):
        """Annotates state machine debug info on frame"""
        y_offset = y_start
        line_height = 18
        ...
        if self.Researcher:
            cv2.putText(frame, f"Researcher: ({int(self.Researcher[0])}, {int(self.Researcher[1])})", (x_start, y_offset),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, color=(0, 255, 0), thickness=1)
        else:
            cv2.putText(frame, "Researcher: NOT DETECTED", (x_start, y_offset),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, color=(0, 0, 255), thickness=1)
        y_offset += line_height
        ...
        if self.goal_location and self.pos_centroid:
            ...
            cv2.putText(frame, f"Rat->Goal: {dist_to_goal:.1f}px (radius:{self.goal_node_radius})", 
                        (x_start, y_offset), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, 
                        color=color, thickness=1)
            y_offset += line_height
        
        # Distance to start node
        if not self.start_trial and self.counter < len(self.start_nodes_locations):
            start_node_center = self.start_nodes_locations[self.counter]
            if self.pos_centroid:
                dist_to_start = points_dist(self.pos_centroid, start_node_center)
                color = (0, 255, 0) if dist_to_start < 60 else (255, 0, 0)
                cv2.putText(frame, f"Rat->Start: {dist_to_start:.1f}px (need:<60px)", 
                            (x_start, y_offset), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, 
                            color=color, thickness=1)
                y_offset += line_height
        
        # Researcher to goal
        if self.Researcher and self.goal_location:
            dist_res_goal = points_dist(self.Researcher, self.goal_location)
            cv2.putText(frame, f"Researcher->Goal: {dist_res_goal:.1f}px", (x_start, y_offset),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, color=(255, 100, 100), thickness=1)
            y_offset += line_height
        
        # Researcher to start node
        if self.Researcher and not self.start_trial and self.counter < len(self.start_nodes_locations):
            start_node_center = self.start_nodes_locations[self.counter]
            dist_res_start = points_dist(self.Researcher, start_node_center)
            color = (0, 255, 0) if dist_res_start <= 40 else (255, 0, 0)
            cv2.putText(frame, f"Researcher->Start: {dist_res_start:.1f}px (need:<=40)", 
                        (x_start, y_offset), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, 
                        color=color, thickness=1)
            y_offset += line_height
        ...
        cv2.putText(frame, f"Researcher->Goal timer: {self.researcher_goal_timer:.1f}s (need:10s)", 
                    (x_start, y_offset), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4, 
                    color=(255, 100, 100), thickness=1)
        y_offset += line_height
        
        if self.record_detections:
            if self.NGL or self.probe:
                minutes = self.timer(start=self.start_time)
        ...
        trial_type_name = {
            '1': 'Standard', '2': 'NGL', '3': 'Probe', '4': 'Free Roaming', '5': 'Free Roaming'
        }.get(str(self.trial_type), f'Unknown({self.trial_type})')
        ...
        if self.start_trial and self.counter < len(self.start_nodes_locations):
            if self.Researcher:
                ...
                dist_res_start = points_dist(self.Researcher, start_node)
                color = (0, 255, 0) if dist_res_start <= 40 else (255, 0, 0)
                cond = f"Res covers start node: {dist_res_start:.1f}<=40? {dist_res_start <= 40}"
        ...
        if self.record_detections:
            ...
            if self.NGL or self.probe:
                minutes = self.timer(start=self.start_time)
                if self.NGL:
                    condition = f"NGL: time>=10min? {minutes >= 10} AND reached={self.reached}"
                else:  # probe
